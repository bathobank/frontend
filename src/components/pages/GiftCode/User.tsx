...
export const GiftCodeUser = () => {
  ...
  const onSubmitGiftCode = () => {
    ...
    userUseGiftCode.mutate({giftcode}, defaultOptionReactQueryResponse(() => {
  ...
  return (
    <Box>
      <Box className="w-full max-w-[550px] m-auto space-y-4 mb-6">
        ...
        <Button variant="theme" fullWidth={true} onClick={onSubmitGiftCode}>Nhận thưởng</Button>
      </Box>
      <Box className="text-center">
        <Text size="xs" className="italic"><span className="text-[#ff55a5] font-bold">LƯU Ý:</span> HỆ THỐNG TỰ ĐỘNG KHÓA/CHẶN CÁC TÀI KHOẢN LẠM DỤNG HOẶC SỬ DỤNG CLONE ĐỂ NHẬP CODE.</Text>
      </Box>
    </Box>
  );
}
