...
const ConfigGame: {key: string; end: string; ratio: string}[] = [
  {key: 'CX', end: '2,4', ratio: 'x3.3'},
  {key: 'LT', end: '5,7', ratio: 'x3.3'},
  {key: 'CT', end: '6,8', ratio: 'x3.3'},
  {key: 'LX', end: '1,3', ratio: 'x3.3'}
]
  ...
  return (
    <Box>
      <Flex className="px-3 py-5 border-b border-b-[#ffffff0d]">
        ...
        <Text custom={true} className="ml-2 text-white">XIÊN SỐ</Text>
      </Flex>
      <Box className="p-3">
          ...
          <tbody>
            {ConfigGame.map((config, index) => (
              <tr key={`tr-game-cltx-${index}`} className={cn(index > 0 ? "border-t border-t-[#ffffff0d]" : '')}>
                ...
                {user && (
                  <td className="min-w-[50px] text-center">
                    <Text className="cursor-pointer hover:underline text-[#ff55a5] select-none" onClick={() => startGame('xien', config.key)}>Chơi</Text>
                  </td>
                )}
              </tr>
            ))}
