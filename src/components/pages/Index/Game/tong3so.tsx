...
export const GameTong3So = ({startGame, gameData}: {startGame: TStartGame, gameData: TGame['tong3so']}) => {
      ...
      <Box className="p-3">
              ...
              return (
                <tr key={`tr-game-cltx-${index}`} className={cn(index > 0 ? "border-t border-t-[#ffffff0d]" : '')}>
                  <td className="py-3 w-[100px] sm:w-[150px]">
                    <Flex className="cursor-pointer select-none" onClick={() => triggerCopyContent(`${user?.nickname ?? 'nickname'} S`)}>
                      <Text custom={true} className="mr-1">{user?.nickname ?? 'nickname'} S</Text>
                  ...
                  {user && (
                    <td className="min-w-[50px] text-center">
                      <Text className="cursor-pointer hover:underline text-[#ff55a5] select-none" onClick={() => startGame('tong3so', 'S')}>Chơi</Text>
                    </td>
                  )}
                </tr>
              );
        ...
        <Box className="py-5">
          <Text className="italic text-[12px] text-center">
            KẾT QUẢ TÍNH BẰNG <span className="text-[#ff55a5]">TỔNG 3 SỐ CUỐI</span> CỦA <span className="text-[#ff55a5]">MÃ GIAO DỊCH BANK</span> KHI CHUYỂN KHOẢN VÀO BANK NHẬN CỦA WEB
          </Text>
        </Box>
      </Box>
    </Box>
