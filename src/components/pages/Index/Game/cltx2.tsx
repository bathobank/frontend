...
export const GameCltx2 = ({startGame, gameData}: {startGame: TStartGame, gameData: TGame['cltx2']}) => {
            ...
            {Object.keys(gameData).map((key, index) => {
              ...
              return (
                <tr key={`tr-game-cltx-${index}`} className={cn(index > 0 ? "border-t border-t-[#ffffff0d]" : '')}>
                  ...
                  {user && (
                    <td className="min-w-[50px] text-center">
                      <Text className="cursor-pointer hover:underline text-[#ff55a5] select-none" onClick={() => startGame('cltx+2', key)}>Chơi</Text>
                    </td>
                  )}
                </tr>
              );
