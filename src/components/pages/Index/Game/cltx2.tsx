...
const ConfigGame: {key: string; end: string; ratio: string}[] = [
  {key: 'TC', end: '0,2,4,6,8', ratio: 'x1.9'},
  {key: 'TL', end: '1,3,5,7,9', ratio: 'x1.9'},
  {key: 'TT', end: '5,6,7,8,9', ratio: 'x1.9'},
  {key: 'TX', end: '0,1,2,3,4', ratio: 'x1.9'}
]
      ...
      <Box className="p-3">
          ...
          <tbody>
            {ConfigGame.map((config, index) => (
              <tr key={`tr-game-cltx-${index}`} className={cn(index > 0 ? "border-t border-t-[#ffffff0d]" : '')}>
                ...
                {user && (
                  <td className="min-w-[50px] text-center">
                    <Text className="cursor-pointer hover:underline text-[#ff55a5] select-none" onClick={() => startGame('cltx+2', config.key)}>Chơi</Text>
                  </td>
                )}
              </tr>
            ))}
        ...
        <Box className="py-5">
          <Text className="italic text-[12px] text-center">
            KẾT QUẢ TÍNH BẰNG <span className="text-[#ff55a5]">TỔNG 2 SỐ CUỐI</span> CỦA <span className="text-[#ff55a5]">MÃ GIAO DỊCH BANK</span> KHI CHUYỂN KHOẢN VÀO BANK NHẬN CỦA WEB
          </Text>
        </Box>
      </Box>
    </Box>
