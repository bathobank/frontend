  ...
  return (
    <Box className="px-3 py-3">
      <Flex
        wrap="wrap"
        justify="between"
        className={cn("bg-[#2b2b31] px-3 lg:hidden", showHeader ? "mb-3" : "")}
      >
        ...
        <Flex className="px-4 rounded-lg bg-[#28282d] border border-[#ffffff0d] shadow-normal overflow-x-auto">
          {MENU.map((menu, index) => (
            <Box
              ...
              className={cn(
                "group text-white min-w-[98px] text-center p-4 border-b-2 border-[transparent] transition-all cursor-pointer",
                menu.slug === gameOpen
                  ? "border-b-[#ff55a5]"
                  : "hover:border-b-[#ff55a5]",
                index > 0 ? "ml-1" : "",
              )}
            >
              <Text
                size="sm"
                className={cn(
                  "text-[#c7c7c7] group-hover:text-white transition-all uppercase",
                  menu.slug === gameOpen ? "text-white" : "",
                )}
              >
                {menu.title}
              </Text>
